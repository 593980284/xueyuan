...
@interface EDSToolClass()
@property (nonatomic, strong) UIView *netTipView;
@end

static EDSToolClass *_instance;
@implementation EDSToolClass{
    BOOL showMess;
    UIView *tipView;
}
//初始化
+ (void)initialize {
    _instance = [EDSToolClass new];
}
...
+ (void)localPhotoWithDelegate:(id)delegate andParentVC:(UIViewController *)VC
{
    ...
    [picker.navigationBar setTintColor:[EDSToolClass getColorWithHexString:@"666666"]];
...
+ (NSString *)formatStr:(NSString *)str withNum:(NSInteger)num withCarakter:(NSString *)carakter fromDist:(NSInteger )dist{
    NSMutableString * changeStr = [NSMutableString stringWithString:[EDSToolClass formatStr:str deleteCarakter:@" " withDStr:@""]];
...
+ (NSDictionary *)parseParams:(NSDictionary *)params{
    ...
    [parameters setValue:[EDSToolClass  md5:result] forKey:@"sig"];
...
+(NSMutableAttributedString *)checkStar:(NSString *)title{
    ...
    if (starRange.location != NSNotFound) {
        [attr addAttribute:NSForegroundColorAttributeName value:[EDSToolClass getColorWithHexString:@"f44336"] range:starRange];
    }
    return attr;
}
...
+ (NSString *)getFamilyNames {
    ...
    return arr[[EDSToolClass getRandomNumberBegin:0 end:arr.count - 1]];
}
/*
 * 上传图片到服务器
 */
...
- (UIView *)netTipView {
    if (!_netTipView) {
        _netTipView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 64)];
